...
interface SidebarProps { data: ScanResult; active: string; onPick: (c: string) => void }
...
export function Sidebar({ data, active, onPick }: SidebarProps): React.JSX.Element {
  const s = data.score;
  return (
    <aside className="flex w-60 shrink-0 flex-col border-r border-white/[0.07] bg-black/20 px-3 py-5">
      ...
      <nav className="flex flex-1 flex-col gap-0.5">
        <NavItem label="Vue d'ensemble" active={active === "Tous"} onClick={() => onPick("Tous")} Icon={LayoutGrid} />
        <div className="my-2 px-2.5 text-[10px] font-semibold uppercase tracking-widest text-white/25">Catégories</div>
