// Détail d'une session (résumé + transcript repliable), partagé entre le graphe Cerveau
// (NodeDetail) et le drawer ouvert depuis une carte de résolution. Charge /api/session/:id
// (résumé) et, à la demande, /api/transcript/:id (transcript nettoyé).
...
import { qualityColor } from "../lib/format.tsx";
...
function SummaryBlock({ s }: { s: SessionSummary }): React.JSX.Element {
  return (
    <div>
      ...
      <Bullets icon={<AlertTriangle size={13} />} title="Erreurs agent" items={s.errors_claude} tone="text-rose-400/90" />
...
export function SessionDetail({ id }: { id: string }): React.JSX.Element {
...
/** Drawer latéral réutilisable : ouvre le détail d'une session depuis n'importe quelle vue. */
export function SessionDrawer({ sessionId, title, subtitle, onClose }:
  { sessionId: string; title: string; subtitle?: string; onClose: () => void }): React.JSX.Element {
  return (
    <motion.div
      initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 40, opacity: 0 }}
      transition={{ type: "spring", stiffness: 320, damping: 32 }}
      className="absolute right-0 top-0 z-30 flex h-full w-[400px] flex-col border-l border-white/[0.08]
      ...
      <div className="flex items-start justify-between gap-3 border-b border-white/[0.07] px-5 py-4">
        <div className="min-w-0">
          {subtitle && (
            <div className="truncate text-[10px] font-semibold uppercase tracking-widest text-fuchsia-300/70">{subtitle}</div>
          )}
          <h3 className="mt-0.5 text-[14px] font-bold leading-tight text-white/90">{title}</h3>
        </div>
        <button onClick={onClose} className="shrink-0 text-white/40 transition hover:text-white/80"><X size={18} /></button>
      </div>
