...
// Deux semaines ISO distinctes (lundis).
...
test("buildEvolution : bucketing hebdo + recurrence S1=0, S2>0", () => {
  ...
  const summaries: SessionSummary[] = [
    summary({ summarizedAt: W1, problems: [problem(a), problem(b)] }), // 2 catégories neuves
    summary({ summarizedAt: W2, problems: [problem(a), problem(b)] }), // les 2 réapparaissent
  ];
...
test("buildEvolution : fitness des champions monte → fitnessTrend improving", () => {
  ...
  const summaries: SessionSummary[] = [
    summary({ summarizedAt: W1, quality_score: 50, problems: [weak] }),
    summary({ summarizedAt: W2, quality_score: 95, problems: [strong] }),
  ];
  const ev = buildEvolution(summaries, buildChampions(summaries));
...
test("buildEvolution : un seul bucket → tendances flat", () => {
  const summaries: SessionSummary[] = [
    summary({ summarizedAt: W1, problems: [problem()] }),
    summary({ summarizedAt: W1 + 3600_000, problems: [problem()] }),
  ];
  const ev = buildEvolution(summaries, buildChampions(summaries));
...
test("buildEvolution : sessions sans summarizedAt ignorées", () => {
  const summaries: SessionSummary[] = [
    summary({ summarizedAt: W1, problems: [problem()] }),
    summary({ summarizedAt: 0, problems: [problem()] }),
    summary({ summarizedAt: -5, problems: [problem()] }),
  ];
  const ev = buildEvolution(summaries, buildChampions(summaries));
  expect(ev.buckets).toHaveLength(1);
