...
- **Reste vague 5** : INC.3b (graduation : principe/champion diplômé → proposition ; journal des propositions + statuts ; 3 toggles auto/manuel par catégorie) puis INC.3c (write-back exécution : snapshot+write région managée OU création SKILL.md via prompt-architect OU archivage + commit git ; gate anti-bloat ; mesure fitness post-révision → revert). **Tension à arbitrer** : la génération LLM (réécriture/création) coûte des tokens → respecter North Star #2/#4 (pas de batch token en arrière-plan) : détection gratuite + génération déclenchée/bornée + application auto/manuel selon toggle.
