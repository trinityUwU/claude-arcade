...
- **Backup config** (`src/config/backup.ts`, demande Chris : auto OK mais backup complète EN PLUS des checkpoints git) : `snapshotConfig()` = tar.gz horodaté complet de la config (CLAUDE.md+rules+commands+skills+settings+.mcp.json) dans `~/.claude/claude-arcade/config-backups`, rétention 30, `listBackups`. Baseline tirée (177 fichiers, 251 Ko). Le write-back inc.3 fera snapshot+commit AVANT chaque écriture (double filet).
- **INC.2 couverture (LIVRÉ)** : `src/config/coverage.ts` `buildCoverage` (déterministe, zéro token) → gaps (classe canonique occ≥4 / ≥2 projets sans skill couvrant par token-overlap≥2) + morts (skill patchable 0 invocation). Calculé **à la demande dans `/api/config/coverage`** (pas dans rebuildInsights → évite couplage consolidate↔scanner). Onglet Config gagne une bascule Fichiers/Couverture. E2E réel : 2 gaps ("mauvais mécanisme invocation outil" 6×, "permission mcp non accordée" 6×) + 39 morts. 101 tests, tsc 0, 0 erreur console. **Signal important** : les 2 gaps réels sont des échecs transitoires/env (anti-patterns CLAUDE.md) → la gate de création inc.3 doit les exclure ; et l'archivage ne doit pas viser les skills à chargement silencieux (agents/llm-*).
- **INC.3a gates affinées (LIVRÉ)** : heuristique échec-env (`ENV_MARKERS` dans coverage.ts → `block:"env-failure"`, non créable) + **liste bannie manuelle** (`banned.ts` + `/api/config/banned`, boutons bannir/réautoriser dans l'app → `block:"banned"`) — les deux combinés (demande Chris). Filtre chargement silencieux (agents, llm-* → `silentLoad`, jamais archivé auto). E2E réel : permission mcp→env-failure auto, mauvais mécanisme invocation→banni manuel (boucle ban complète), 20/39 morts archivables. 104 tests, tsc 0, 0 erreur console.
- **INC.3b graduation + journal + réglages (LIVRÉ, token-free)** : `graduation.ts` `buildGraduation` (patch=principe conf≥0.7+jugé+non contesté→skill ciblé par token-match ; create=gap créable ; archive=mort archivable) + `mergeWithJournal` (live↔journal). `proposals-store.ts` (journal statuts). `settings.ts` (kill-switch + 3 toggles + cap maxPerCycle=3 anti-batch). Routes `/api/config/{proposals,settings}`. Sous-onglet « Auto-évolution » (toggles + propositions). E2E réel : 20 propositions archive, **0 create** (les 2 gaps bloqués par gates → chaîne complète prouvée). 109 tests, tsc 0, 0 erreur console. Bug clé dupliquée `archive:skills` corrigé (fallback nom skill premier niveau dans scan.ts).
- **ARBITRAGE TOKENS (Chris a choisi auto-total)** : contredit North Star #2/#4 → signalé. Implémenté en **auto-total PLAFONNÉ** : auto sans clic MAIS cap `maxPerCycle`, jamais pendant backfill (`ARCADE_LOOP_ACTIVE`), kill-switch global. Chris peut retirer le cap s'il insiste (signalé).
- **INC.3c exécution write-back (LIVRÉ)** : `evolve-prompt.ts`+`evolve.ts` (génération patch=réécriture sémantique / create=SKILL.md via `claude -p` isolé, **générateur injectable** → testable sans tokens ; gate anti-bloat ×1.25). `apply.ts` (snapshot→générer/déplacer→write→`commitPaths` add -A→journal ; archive=move vers `skills/.archived/`, token-free). `evolve-job.ts` `runEvolution` (pending autorisées plafonnées maxPerCycle, jamais en batch via ARCADE_LOOP_ACTIVE, kill-switch) **branché sur `consolidateSession`** (temps réel, gardé try/catch). API `/api/config/{proposals/apply, evolve}` + boutons appliquer/Lancer. **`autoArchive` défaut OFF** (0-invoc trop faible pour supprimer des skills faits main — signalé). 4 tests apply en config temp (archive/patch/create/anti-bloat), 113 tests, tsc 0, E2E réel (UI rendu, archivage OFF visible, 0 erreur). **Vraie config jamais touchée pendant le dev** (apply testé en temp uniquement).
- **VAGUE 5 COMPLÈTE** (inc.1→3c). La boucle est fermée jusqu'à la config source. Filets : backup tar complète + git versionné + kill-switch + cap anti-batch + gates (env-failure/banni/silent-load) + anti-bloat. **Comportement live** : `autoGenerate` ON → à chaque SessionEnd réel, `runEvolution` peut générer+appliquer jusqu'à 3 patch/create sur les vrais skills (archive OFF). État actuel : 0 patch/create candidat (gaps bloqués, aucun principe jugé+matchant) → rien ne s'écrira tant que la matière ne mûrit pas.
- **Mesure post-révision (LIVRÉ)** : `revisions.ts` `assessRevisions` = qualité moyenne des sessions avant/après l'application d'une proposition (fenêtre 14j, ≥3 sessions/côté, marge 5 pts) → `regression` flaggée. `/api/config/revisions` + `/api/config/revert` (revert manuel d'un commit). Section « Révisions appliquées » dans l'onglet (masquée tant qu'aucune écriture). 4 tests (117 total). La boucle se mesure elle-même : une réécriture qui dégrade les sessions suivantes est signalée + revertable en un clic.
- **VAGUE 5 INTÉGRALEMENT LIVRÉE** (inc.1→3c + mesure). 117 tests, tsc 0, chaque incrément validé E2E réel. Repo GitHub privé créé + push.
