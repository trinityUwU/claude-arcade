...
- **Reste vague 5** : INC.3c exécution write-back — génération LLM (réécriture skill via prompt-architect / création SKILL.md / archivage), gate anti-bloat, snapshot+write+commit (double filet déjà prêt), auto-runner plafonné branché sur consolidation, mesure fitness post-révision→revert. **Rien n'écrit encore dans les vrais skills : les propositions restent `pending`, aucun chemin d'application = sûr.**
