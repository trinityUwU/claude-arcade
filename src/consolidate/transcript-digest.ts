...
export interface TranscriptDigest {
  ...
  endTs: number;   // epoch ms du dernier message (borne haute de la fenêtre), 0 si inconnu
}
...
export function digestTranscript(lines: TranscriptLine[]): TranscriptDigest {
  ...
  const out: string[] = [];
  let project = "", sessionId = "", messageCount = 0, startTs = 0, endTs = 0;
  for (const line of lines) {
    ...
    if (line.timestamp) {
      ...
      if (Number.isFinite(ts) && ts > 0) { if (!startTs) startTs = ts; endTs = ts; }
    }
    messageCount++;
  ...
  return {
    project, sessionId, models: [...models], messageCount,
    text: capHeadTail(out, TOTAL_CHARS), startTs, endTs,
  };
}
