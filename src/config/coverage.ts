...
import type { ConfigEntry, CoverageReport, CoverageGap, CoverageDeadSkill } from "./types.ts";
...
function findGaps(
  ...
  skillTokens: Set<string>[], byClass: Map<string, Set<string>>,
): CoverageGap[] {
...
function findDead(entries: ConfigEntry[], usage: SkillUsage[]): CoverageDeadSkill[] {
  ...
  return entries
    ...
    .map((x) => ({ name: x.e.name, relPath: x.e.relPath, invocations: x.u?.count ?? 0, sessions: x.u?.sessions ?? 0 }));
}
...
export function buildCoverage(
  ...
  return {
    ...
    gaps: findGaps(registry, champions, skillTokens, projectsByClass(summaries)),
